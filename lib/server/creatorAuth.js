...
export async function getCreatorContext() {
  ...
  const { creator, promo, promoIsNew } = await ensureCreatorForUser({
    clerkUserId: userId,
    email,
    name: fullName,
  });

  // Sync new promo codes to the Go backend so the same code works in the app.
  if (promoIsNew && promo?.code) {
    syncPromoCodeToBackend(creator, promo.code).catch(() => {});
  }

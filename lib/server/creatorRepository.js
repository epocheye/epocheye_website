...
export const MIN_PAYOUT_INR = 500;   // ₹500 minimum payout
export const MIN_PAYOUT_USD = 6;     // ~$6 minimum payout (keep for backward compat)
...
export async function createPayoutRequest({ creatorId, amount, upiId, currency }) {
  const sql = getSql();
  const rows = await sql`
    INSERT INTO payout_requests (creator_id, amount, currency, status, upi_id)
    VALUES (${creatorId}, ${amount}, ${currency ?? "INR"}, 'processing', ${upiId})
    RETURNING *
  `;

...
import { CREATOR_TERMS_VERSION, rateForSale } from "@/lib/server/creatorProgram";
...
export function toPublicProfile(row) {
  ...
  return {
    ...publicFields,
    terms_current: creator.terms_version === CREATOR_TERMS_VERSION && !!creator.terms_accepted_at,
  };
}
...
export async function createCreator({ clerkUserId, name, email }) {
  const sql = getSql();
  // New creators start on the admin's default commission (Settings), falling
  // back to the column default when it is unset.
  const rows = await sql`
    ...
    VALUES (
      ...
      COALESCE((SELECT default_commission_rate FROM admin_settings WHERE id = 1), 10),
      NULL,
      NULL,
      NULL,
      NULL,
...
export async function updateCreator(id, fields) {
  ...
  for (const [key, rawValue] of Object.entries(fields || {})) {
    ...
    // JSONB columns take a serialized value.
    values.push(key === "application" && rawValue != null ? JSON.stringify(rawValue) : rawValue ?? null);
  }
...
export async function listActivePromoCodesByCreator(creatorId) {
  const sql = getSql();
  return sql`
    SELECT *
    FROM promo_codes
...
export async function markPromoCodeSynced(promoId) {
  const sql = getSql();
  await sql`UPDATE promo_codes SET backend_synced_at = NOW() WHERE id = ${promoId}`;
}
...
/**
 * Turn a caller's IP into a same-day-only, non-reversible token.
 *
 * WHAT THIS PRESERVES: the only thing the raw IP was ever used for is telling
 * two clicks apart — de-duplicating a refresh, spotting one machine clicking a
 * creator's link two hundred times. A stable hash does that exactly as well as
 * the address itself.
 *
 * WHAT IT GIVES UP, deliberately: the date is mixed into the digest, so the same
 * visitor hashes differently tomorrow. That makes cross-day tracking of an
 * individual impossible while leaving same-day fraud detection untouched, which
 * is the only window that question is ever asked in.
 *
 * NO SALT CONFIGURED → NULL, not a weak hash. An unsalted digest of an IPv4
 * address is trivially reversible by brute force (there are only ~4bn of them),
 * so a missing secret must mean "store nothing", never "store something that
 * merely looks hashed".
 */
...
/** ios | android | desktop — all `detectPlatform` ever derived from the UA. */
 ...
 *
 * The two identifying columns are written in reduced form. Nothing in either
 * repo has ever READ `ip_address` or `user_agent` back — they are write-only
 * columns, so storing a day-scoped hash and a three-value platform string costs
 * no existing feature and removes the only place a non-user who merely clicked a
 * link had identifying data written about them.
 */
...
/**
 * Records one attributed sale. planAmount is the LIST price (commission base,
 * whatever discount the customer got). When commissionRate is omitted, the
 * creator's tier rate for this sale is used: the n-th qualifying (non-reversed)
 * sale earns rateForSale(n). Returns { commissionAmount, commissionRate }.
 */
  ...
  if (!Number.isFinite(rate)) {
    const prior = await sql`
      ...
      WHERE creator_id = ${creatorId}
        AND status <> 'reversed'
    `;
    rate = rateForSale(Number(prior[0]?.count ?? 0) + 1);
  }
  const commissionAmount = Number(((planAmount * rate) / 100).toFixed(2));
  // Prefer the discount the backend actually applied; the rate-based estimate
  // is only for legacy callers that don't send one.
  const discountAmount = Number.isFinite(actualDiscount)
    ? Number(actualDiscount.toFixed(2))
    : Number(((planAmount * customerDiscountRate) / 100).toFixed(2));
...
/** Monuments shown on the creator page: [{ name, place }], admin-edited. */
export function normalizeMonuments(value) {
  const list = Array.isArray(value) ? value : [];
  return list
    .map((m) => ({
      name: String(m?.name ?? "").trim(),
      place: String(m?.place ?? "").trim(),
      // App site slug (monuments.slug); required for creator site assignments.
...
export async function getAdminSettings() {
  ...
  return {
    ...
    offer_enabled: row.offer_enabled !== false,
    offer_claimed: Number(row.offer_claimed ?? 120),
    offer_total: Number(row.offer_total ?? 500),
    offer_promo_code: row.offer_promo_code ?? "BLR500",
    creator_monuments: normalizeMonuments(row.creator_monuments),
    max_creators: Number(row.max_creators ?? 100),
    assignment_days: Number(row.assignment_days ?? 14),
    assignment_sales_target: Number(row.assignment_sales_target ?? 10),
  };
}
...
export async function updateAdminSettings(fields) {
  ...
  await sql`
    UPDATE admin_settings SET
      ...
      offer_enabled            = COALESCE(${updates.offer_enabled ?? null}::boolean, offer_enabled),
      offer_claimed            = COALESCE(${updates.offer_claimed ?? null}::int, offer_claimed),
      offer_total              = COALESCE(${updates.offer_total ?? null}::int, offer_total),
      offer_promo_code         = COALESCE(${updates.offer_promo_code ?? null}::text, offer_promo_code),
      creator_monuments        = COALESCE(${
        updates.creator_monuments ? JSON.stringify(updates.creator_monuments) : null
      }::jsonb, creator_monuments),
      max_creators             = COALESCE(${updates.max_creators ?? null}::int, max_creators),
      assignment_days          = COALESCE(${updates.assignment_days ?? null}::int, assignment_days),
      assignment_sales_target  = COALESCE(${updates.assignment_sales_target ?? null}::int, assignment_sales_target),
      updated_at               = NOW()
    WHERE id = 1
  `;
}
...
export async function listAllPayouts({ status, limit = 50, offset = 0 } = {}) {
  const sql = getSql();

  const normalizedStatus =
    typeof status === "string" && status !== "" && status !== "all" ? status : null;

  const safeLimit = Math.max(1, Math.min(200, Number(limit) || 50));
  const safeOffset = Math.max(0, Number(offset) || 0);

  const countRows = normalizedStatus
    ? await sql`
  ...
  const rows = normalizedStatus
    ? await sql`
        ...
        WHERE pr.status = ${normalizedStatus}
...
export async function listAdminContent({ status, platform, limit = 50, offset = 0 } = {}) {
  const sql = getSql();

  const normalizedStatus =
    typeof status === "string" && status !== "" && status !== "all" ? status : null;
  const normalizedPlatform =
    typeof platform === "string" && platform !== "" && platform !== "all"
      ? platform
      : null;
  ...
  const countRows = normalizedStatus && normalizedPlatform
    ? await sql`
        SELECT COUNT(*)::int AS total
        FROM content_submissions
        WHERE status = ${normalizedStatus}
  ...
  const rows = normalizedStatus && normalizedPlatform
    ? await sql`
        SELECT
          cs.*,
          c.name AS creator_name,
        ...
        WHERE cs.status = ${normalizedStatus}
          AND cs.platform = ${normalizedPlatform}
    ...
    : normalizedStatus
      ...
      : normalizedPlatform
        ? await sql`
            ...
            FROM content_submissions cs
            LEFT JOIN creators c ON c.id = cs.creator_id
            WHERE cs.platform = ${normalizedPlatform}
            ORDER BY cs.submitted_at DESC
            LIMIT ${safeLimit}
            OFFSET ${safeOffset}
          `

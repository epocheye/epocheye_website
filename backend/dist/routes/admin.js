...
router.put("/creators/:id", adminAuth_1.requireAdmin, async (req, res) => {
    ...
    const updated = await (0, creatorService_1.updateCreator)(req.params["id"], result.data);
...
router.put("/payouts/:id", adminAuth_1.requireAdmin, async (req, res) => {
    ...
    await (0, payoutService_1.updatePayoutStatus)(req.params["id"], result.data.status);
...
router.put("/content/:id", adminAuth_1.requireAdmin, async (req, res) => {
    ...
    await supabase_1.supabase.from("content_submissions").update(updates).eq("id", req.params.id);

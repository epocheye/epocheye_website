...
export async function getStatsOverview(creatorId: string): Promise<StatsOverview> {
  const now = new Date();
  ...
  const totalRequestedPayouts = (payoutData ?? []).reduce(
  ...
  const confirmedEarnings = (confirmedData ?? []).reduce(

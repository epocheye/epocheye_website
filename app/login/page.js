...
import { Mail, Lock, ArrowRight, Sparkles, ShieldCheck, Landmark } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const nextPath = data?.user?.site_id 
        ? `/dashboard/analytics?siteId=${data.user.site_id}` 
        : "/dashboard/analytics";
        ...
        <div className="grid w-full gap-6 lg:gap-8 lg:grid-cols-[1.05fr_0.95fr]">
          <div className="relative overflow-hidden rounded-2xl lg:rounded-3xl border border-white/10 bg-linear-to-b from-white/10 via-white/5 to-black/40 p-6 sm:p-8 lg:p-10 shadow-[0_30px_120px_rgba(0,0,0,0.45)] backdrop-blur-xl">
            ...
            <div className="flex items-center gap-3 text-xs sm:text-sm text-emerald-200">
              <Landmark className="size-4 sm:size-5" /> <span className="hidden sm:inline">Tourism Operator / ASI Portal</span><span className="sm:hidden">ASI Portal</span>
            </div>
            <h1 className="mt-4 text-2xl sm:text-3xl font-semibold text-white">Secure Login</h1>
            ...
            <form onSubmit={handleSubmit} className="mt-6 sm:mt-8 space-y-4 sm:space-y-5">
              ...
              <label className="block space-y-2 text-xs sm:text-sm">
                ...
                <div className="flex items-center gap-2 sm:gap-3 rounded-xl border border-white/10 bg-black/40 px-3 sm:px-4 py-2.5 sm:py-3 focus-within:border-emerald-400/60">
                  <Lock className="size-4 sm:size-5 text-emerald-300 shrink-0" />
                  <input
                    type="password"
                    name="password"
                    value={form.password}
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0 text-xs sm:text-sm text-zinc-400">
                <label className="inline-flex items-center gap-2">
                  ...
                  />
                  <span className="text-xs sm:text-sm">Keep me signed in</span>
                </label>
                <Link href="#" className="text-emerald-300 hover:text-emerald-200 text-xs sm:text-sm">Need help?</Link>
              </div>

              <button
                type="submit"
                disabled={loading}
                className="group flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-400 px-4 py-2.5 sm:py-3 text-sm font-semibold text-black shadow-[0_12px_40px_rgba(52,211,153,0.35)] transition hover:bg-emerald-300 disabled:opacity-60 disabled:cursor-not-allowed">
          ...
          <div className="relative flex flex-col justify-center gap-4 sm:gap-6 rounded-2xl lg:rounded-3xl border border-white/10 bg-linear-to-b from-white/5 via-white/5 to-black/60 p-6 sm:p-8 shadow-[0_20px_80px_rgba(0,0,0,0.45)] backdrop-blur-xl">
            <div className="pointer-events-none absolute -top-14 right-10 h-28 w-28 rounded-full bg-emerald-400/20 blur-3xl" />
            <div className="flex items-start gap-3 text-zinc-300">
              <div className="flex size-10 sm:size-12 items-center justify-center rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-200 shrink-0">
                <ShieldCheck className="size-5 sm:size-6" />
              </div>
              <div>
                <p className="text-xs sm:text-sm text-emerald-200">Secure access</p>
                <p className="text-base sm:text-lg font-semibold text-white">JWT-protected dashboards</p>
                <p className="text-xs sm:text-sm text-zinc-400">Connects to Supabase-backed auth</p>
              </div>
            </div>

            <div className="space-y-2 sm:space-y-3 text-xs sm:text-sm text-zinc-200">
              <p className="text-base sm:text-lg font-semibold text-white">Single portal for operators</p>
              <p className="text-zinc-400">
                Streamlined access for Archaeological Survey of India teams, tourism boards, and site operators.
                <span className="hidden sm:inline"> Switch to sign-up if you need a fresh operator profile.</span>
              </p>
              <ul className="space-y-2 text-zinc-300 text-xs sm:text-sm">
                <li className="flex items-center gap-2"><ShieldCheck className="size-3 sm:size-4 text-emerald-300 shrink-0" /> SSO-ready guardrails</li>
                <li className="flex items-center gap-2"><ShieldCheck className="size-3 sm:size-4 text-emerald-300 shrink-0" /> Device-level trust signals</li>
                <li className="flex items-center gap-2"><ShieldCheck className="size-3 sm:size-4 text-emerald-300 shrink-0" /> Encrypted session storage</li>
              </ul>
            </div>

            <div className="flex flex-col gap-3">
              <Link href="/signup" className="group inline-flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 sm:py-3 text-xs sm:text-sm font-semibold text-white transition hover:border-emerald-300/60">

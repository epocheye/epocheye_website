              ...
              <tbody>
                {filtered.map((p) => (
                  <tr key={p.id} className="border-b border-white/3 hover:bg-white/2 transition-colors">
                    ...
                    <td className="px-5 py-3 text-right text-white/70 font-medium">
                      {currencySymbol(p.currency)}{Number(p.amount).toFixed(2)}{" "}
                      <span className="text-white/30 text-xs">{p.currency}</span>
                    </td>
                    <td className="px-5 py-3 text-white/40 text-xs font-mono">{p.upi_id ?? "—"}</td>
                    ...
                    <td className="px-5 py-3 text-right">
                      ...
                      {p.status === "processing" && (
                        <button
                          ...
                          className="px-2.5 py-1 text-xs border border-white/10 text-white/50 rounded-md hover:bg-white/5 disabled:opacity-40 transition-colors">
                          Mark paid
                        </button>
                      )}
                    </td>

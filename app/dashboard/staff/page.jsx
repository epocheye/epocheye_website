...
export default function StaffAdminPage() {
  ...
  useEffect(() => {
    ...
    const stored = typeof window !== "undefined" ? window.localStorage.getItem("epocheye_site_id") : null;
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setStaff((prev) => [...prev, data.data].sort((a, b) => a.name.localeCompare(b.name)));
      setForm(emptyForm);
    } catch (err) {
  ...
  const StaffStatus = ({ active }) => (
    <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs ${
      ...
      {active ? <CheckCircle2 className="size-3.5" /> : <AlertCircle className="size-3.5" />}
  ...
  const NoSiteSelected = () => (
    <Card className="border-white/10 bg-white/5">
      ...
      <CardContent className="space-y-2 text-sm text-zinc-300">
        <p>No site found in context. Open analytics for a site or append ?siteId= in the URL.</p>
      </CardContent>
    </Card>
  );
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-zinc-400">Add, edit, or remove staff for the selected site.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button
              variant="outline"
          ...
          <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
            <Card className="border-white/10 bg-white/5">
              <CardHeader className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                ...
                <div className="text-xs text-zinc-400">Site ID: {siteId}</div>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                {loading ? (
                  ...
                  <p className="text-sm text-zinc-400">No staff in this site yet. Add a team member below.</p>
                ) : (
                  <>

      ...
      <div className="bg-[#0d0d0d] border border-white/5 rounded-xl p-6">
        ...
        {!upiId && (
          <p className="text-xs text-white/30 mt-3">
            Go to <a href="/creators/dashboard/settings" className="underline hover:text-white/60">Settings</a> to add your UPI ID first.
          </p>
        )}
      </div>
                ...
                {payouts.map((p) => (
                  <tr key={p.id} className="hover:bg-white/[0.015] transition-colors">
                    ...
                    <td className="py-3.5 px-2">
                      <span className={`text-xs font-medium ${STATUS_STYLES[p.status] ?? "text-white/50"}`}>
                        {p.status}
                      </span>
                    </td>
                    <td className="py-3.5 px-2 text-white/40 font-mono text-xs">{p.upi_id ?? "—"}</td>
